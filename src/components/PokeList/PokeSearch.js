...
const PokeSearch = () => {
    ...
    useEffect(() => {
    GetPokemonList().then(response =>{return response.results}).then(pokes => {
    ...
    return (
        <div>
            <div>
                <input type='text' value={currentInputValue} onChange={handleOnChange}/></div>
            <div>
                {searchItems.map(pokemon => <div>{pokemon}</div>)}
            </div>
